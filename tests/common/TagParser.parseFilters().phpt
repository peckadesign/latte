...
function format(string $str, bool $content = false)
{
	...
	$node = $parser->parseFilters();
	if (!$parser->isEnd()) {
		throw $parser->stream->buildUnexpectedException();
	}
	return $content
		? $node->printContent(new PrintContext, '@')
...
test('FilterInfo aware modifiers', function () {
	...
	Assert::same('LR\Filters::convertTo($ʟ_fi, \'html\', $this->filters->filterContent(\'mod2\', $ʟ_fi, $this->filters->filterContent(\'mod1\', $ʟ_fi, @)))', format('|mod1|mod2|escape', true));
});
